...
export interface TranslationKeys {
    ...
    timeEstimation: {
        [key in keyof typeof translationKeys.timeEstimation]: key extends 'ltSecond' | 'second' | 'minute' | 'hour' | 'day' | 'month' | 'year' | 'centuries' ? string : string | PluralizationFunction;
    };
}
export type L33tTableDefault = typeof l33tTableDefault;
export type LooseObject = Record<string, any>;
export type Pattern = 'dictionary' | 'spatial' | 'repeat' | 'sequence' | 'wordSequence' | 'regex' | 'date' | 'bruteforce' | 'separator' | string;
...
export type MatchExtended = DictionaryMatch | L33tMatch | SpatialMatch | RepeatMatch | SequenceMatch | WordSequenceMatch | RegexMatch | DateMatch | BruteForceMatch | SeparatorMatch | Match;
...
export declare abstract class MatcherBaseClass {
    protected options: Options;
    constructor(options: Options);
    abstract match(matchOptions: MatchOptions): MatchExtended[] | Promise<MatchExtended[]>;
}
export type MatcherConstructor<T extends MatcherBaseClass = MatcherBaseClass> = new (options: Options) => T;
